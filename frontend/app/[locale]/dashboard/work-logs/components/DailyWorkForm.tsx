...
export default function DailyWorkForm() {
  ...
  const submitKarnet = async () => {
    ...
    const payloadEntries = Object.entries(entries)
      ...
      .filter(([_, val]) => val.reg > 0 || val.over > 0)
      .map(([id, val]) => ({
        workerId: id,
  ...
  return (
    <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
          ...
          <tbody className="divide-y divide-slate-100">
            {workers.map(w => (
              <tr key={w.id} className="hover:bg-blue-50/30 transition-colors">
                <td className="p-6">
                  <p className="font-bold text-slate-800">{w.firstName} {w.lastName}</p>
                  <p className="text-[10px] text-slate-400 italic">{w.position || "Worker"}</p>
                </td>
                <td className="p-6">
                  <input 
                    type="number" 
      ...
      <div className="p-6 bg-slate-50 border-t flex justify-between items-center">
        <p className="text-xs text-slate-400 font-medium italic">{t("defaultNote")}</p>
        <button 
